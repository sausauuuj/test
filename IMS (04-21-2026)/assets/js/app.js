...
function fillAssetWizardForBatchEdit(asset, batchAssets = []) {
    ...
    $('.asset-edit-serial-field').removeClass('hidden');
...
function renderAssetsDirectoryTable(rows = appState.assetDirectory) {
    ...
    const groupedRows = [];
    const groupedByPar = new Map();

    assets.forEach((asset) => {
        const parId = Number(asset.par_id || 0);
        const groupKey = parId > 0 ? `par-${parId}` : `asset-${asset.id || Math.random()}`;

        if (!groupedByPar.has(groupKey)) {
            groupedByPar.set(groupKey, {
                ...asset,
                assets_count: 0,
                property_names: [],
                property_types: [],
            ...
            groupedRows.push(groupedByPar.get(groupKey));
        }

        const group = groupedByPar.get(groupKey);
        group.assets_count += 1;
        group.source_assets.push(asset);

        const propertyName = String(asset.property_name || '').trim();
        if (propertyName && !group.property_names.includes(propertyName)) {
            group.property_names.push(propertyName);
        }

        const propertyType = String(asset.property_type || '').trim();
        if (propertyType && !group.property_types.includes(propertyType)) {
            group.property_types.push(propertyType);
        }

        const propertyId = String(asset.property_id || '').trim();
        if (propertyId && !group.property_ids.includes(propertyId)) {
            group.property_ids.push(propertyId);
        }
    });

    $('#assetsDirectoryMeta').text(formatRecordCountLabel(groupedRows.length));

    // Update pagination state
    paginationState.assets.totalRows = groupedRows.length;
    ...
    // Get paginated rows
    const paginatedAssets = getPaginatedRows(groupedRows, 'assets');
    const startIndex = (paginationState.assets.currentPage - 1) * paginationState.assets.rowsPerPage + 1;

    $('#assetsDirectoryBody').html(
        paginatedAssets.map((asset, index) => `
            <tr class="${asset.property_ids.some((propertyId) => highlightedIds.has(String(propertyId || '').trim())) || (highlightedParNumber && String(asset.par_number || '').trim() === highlightedParNumber) ? 'assets-directory-row--highlight' : ''}" data-id="${escapeHtml(asset.id || '')}" data-par-id="${escapeHtml(asset.par_id || '')}" data-par-number="${escapeHtml(asset.par_number || '')}">
                ...
                <td class="px-4 py-4 text-slate-700">
                    <div class="manage-asset-name">${escapeHtml(asset.property_names[0] || asset.property_name || 'No property name')}</div>
                </td>
                <td class="px-4 py-4 text-slate-700">
                    <div class="manage-par-number">${escapeHtml(asset.property_types.join(', ') || asset.property_type || 'Not set')}</div>
                </td>
                ...
                <td class="px-4 py-4 text-slate-700 text-center">${escapeHtml(normalizeAssetFundingSource(asset.funding_source))}</td>
                <td class="px-4 py-4 text-slate-700 text-center">
                    <span class="status-pill">${escapeHtml(String(asset.assets_count || 0))}</span>
                </td>
                <td class="px-4 py-4 text-center">${renderRowViewAction('view-par-assets', asset.id, 'View PAR assets')}</td>
            </tr>
        `).join('')
    );
...
function setInventoryRequestType(requestType) {
    ...
    syncInventoryCategoryOptions(normalized);
}
...
function setInventoryFormStage(stage) {
    ...
    $('#inventoryStep3Section').toggleClass('hidden', normalized !== 'step3');

    keys.forEach((key, index) => {
        const state = states[index];
        const $step = $(`#inventoryProgressTracker .asset-progress-step[data-inventory-progress-step="${key}"]`);
        $step.removeClass('is-active is-complete is-pending');
        $step.addClass(`is-${state}`);
    });

    $('#inventoryProgressTracker .inventory-progress__line').each(function (index) {
        $(this).toggleClass('is-complete', index < completeLineCount);
    });
}
...
function validateInventoryStepTwo() {
    const division = String($('#inventoryForm [name="division"]').val() || '').trim();
    const officerId = Number($('#inventoryForm [name="officer_id"]').val() || 0);
    const errors = {};
    ...
    if (officerId <= 0) {
        errors.officer_id = 'Choose an officer name.';
    }
...
function syncInventoryCategoryOptions(requestType, selectedCategory = '') {
    ...
    const $select = $('#inventoryForm [name="category_select"]');
    const currentCategory = String(selectedCategory || $('#inventoryForm [name="category"]').val() || '').trim();

    if (!$select.length) {
        return;
    }

    const optionMarkup = ['<option value="">Select category</option>']
        .concat(categories.map((category) => `<option value="${escapeHtml(category)}">${escapeHtml(category)}</option>`))
        .concat('<option value="__custom__">+ Add Category</option>')
        .join('');

    $select.html(optionMarkup);

    const hasKnownCategory = categories.includes(currentCategory);
    $select.val(hasKnownCategory ? currentCategory : (currentCategory ? '__custom__' : ''));
    $('#inventoryForm [name="category"]').val(hasKnownCategory ? currentCategory : currentCategory);
    $('#inventoryCustomCategoryWrap').toggleClass('hidden', $select.val() !== '__custom__');
    $('#inventoryForm [name="custom_category"]').val(hasKnownCategory ? '' : currentCategory);
    refreshSearchableSelect($select);
}

function syncInventoryCategoryValue() {
    const $select = $('#inventoryForm [name="category_select"]');
    const selected = String($select.val() || '').trim();
    const customCategory = String($('#inventoryForm [name="custom_category"]').val() || '').trim();
    const isCustom = selected === '__custom__';

    $('#inventoryCustomCategoryWrap').toggleClass('hidden', !isCustom);
    $('#inventoryForm [name="category"]').val(isCustom ? customCategory : selected);
}

function refreshInventoryStockLimitHint() {
    const quantity = Math.max(0, Number($('#inventoryForm [name="quantity_issued"]').val() || 0));
    const mediumThreshold = quantity > 0 ? Math.max(1, Math.ceil(quantity / 2)) : 0;
    const lowThreshold = quantity > 0 ? Math.max(1, Math.ceil(quantity / 4)) : 0;

    $('#inventoryStockLimitHint').text(
        quantity > 0
            ? `Medium alert starts at ${mediumThreshold} remaining. Low alert starts at ${lowThreshold} remaining.`
            : 'If current stock reaches half of the total quantity, status becomes medium. If only one-fourth remains, status becomes low.'
    );
}
...
function resetInventoryForm() {
    ...
    $form.find('[name="custom_category"]').val('');
    loadInventoryOfficers('');
    setInventoryRequestType('');
    setInventoryFormStage('step1');
    syncInventoryCategoryOptions('');
    refreshInventoryStockLimitHint();
...
function openInventoryModal(item = null) {
    ...
    if (item) {
        ...
        $('#inventoryForm [name="funding_source"]').val(normalizeAssetFundingSource(item.funding_source || ''));
        $('#inventoryForm [name="division"]').val(item.division || '');
        $('#inventoryForm [name="issued_at"]').val(item.issued_at || '');
        $('#inventoryForm [name="category"]').val(item.category || '');
        ...
        syncInventoryCategoryOptions(item.request_type || '', item.category || '');
    ...
    $('#inventoryDetailsContent').html(`
        <div class="inventory-details-body">
            <div class="inventory-details-section">
                <h4 class="inventory-details-section-title">Issuance Summary</h4>
                <div class="inventory-details-grid">
                    ...
                    <div class="inventory-detail-row">
                        <div class="inventory-detail-label">Responsibility Center</div>
                        <div class="inventory-detail-value">${escapeHtml(item.division || 'N/A')}</div>
                    </div>
                    <div class="inventory-detail-row">
                        <div class="inventory-detail-label">Officer</div>
                        <div class="inventory-detail-value">${escapeHtml(item.officer_name || 'N/A')}</div>
                    </div>
                    <div class="inventory-detail-row">
                        <div class="inventory-detail-label">Date Issued</div>
                    ...
                    <div class="inventory-detail-row">
                        <div class="inventory-detail-label">Quantity Issued</div>
                    ...
                    <div class="inventory-detail-row">
                        <div class="inventory-detail-label">Stock Limit</div>
                        <div class="inventory-detail-value">${escapeHtml(String(item.stock_limit || 0))}</div>
                    </div>
                    <div class="inventory-detail-row">
                        <div class="inventory-detail-label">Stock Status</div>
                        <div class="inventory-detail-value">${escapeHtml(item.stock_status_label || 'In Stock')}</div>
                    </div>
                    <div class="inventory-detail-row">
                        <div class="inventory-detail-label">Status Remarks</div>
                        <div class="inventory-detail-value">${escapeHtml(item.stock_remark || 'Stock level is healthy.')}</div>
                    </div>
                </div>
            </div>

            <div class="inventory-details-section">
                <h4 class="inventory-details-section-title">Stock Movement History</h4>
                        ...
                        <tbody>
                            ${movements.length ? movements.map((movement) => \`
                                <tr>
                                    ...
                                    <td>\${escapeHtml(String(movement.quantity || 0))}</td>
                                    <td>\${escapeHtml(String(movement.previous_stock || 0))}</td>
                                    <td>\${escapeHtml(String(movement.current_stock || 0))}</td>
                                    <td>\${escapeHtml(movement.notes || 'No notes')}</td>
                                    <td>\${escapeHtml(formatInventoryTimestamp(movement.created_at))}</td>
                                </tr>
                            \`).join('') : '<tr><td colspan="6" class="text-center text-slate-500">No stock records yet.</td></tr>'}
                        </tbody>
                    </table>
                </div>
            </div>
...
function renderParAssetSelection(rows = [], sourceAsset = null) {
    ...
    $('#assetParSelectionBody').html(
        assets.map((asset, index) => `
            <tr class="cursor-pointer transition hover:bg-blue-50/70 ${checkedIds.has(Number(asset.id || 0)) ? 'is-selected' : ''}" data-id="${escapeHtml(asset.id || '')}">
                <td class="px-4 py-4 text-center asset-par-selection-row-check-wrap">
                    <input type="checkbox" class="asset-par-selection-checkbox asset-par-selection-row-check" data-id="${escapeHtml(asset.id || '')}" ${checkedIds.has(Number(asset.id || 0)) ? 'checked' : ''} aria-label="Select ${escapeHtml(asset.property_name || 'asset')}">
                </td>
...
function syncParSelectionActions() {
    ...
    $('#bulkEditAssetParButton span').text(
        mode === 'edit'
            ? (readyCount ? `Proceed Bulk Edit (${selectedCount})` : 'Cancel')
            : 'Bulk Edit'
    );
    $('#bulkDeleteAssetParButton span').text(
        mode === 'delete'
            ? (readyCount ? `Proceed Bulk Delete (${selectedCount})` : 'Cancel')
            : 'Bulk Delete'
    );
    $('#bulkEditAssetParButton').toggleClass('is-active', mode === 'edit');
    $('#bulkDeleteAssetParButton').toggleClass('is-active', mode === 'delete');
...
$(function () {
    ...
    $doc.on('click.app', '#assetsDirectoryBody .view-par-assets', function () {
    ...
    $doc.on('click.app', '#inventoryStep1Next', function (event) {
        ...
        if (validateInventoryStepOne()) {
            setInventoryFormStage('step2');
        }
    });

    $doc.on('click.app', '#inventoryStep2Back', function (event) {
        event.preventDefault();
        setInventoryFormStage('step1');
    });

    $doc.on('click.app', '#inventoryStep2Next', function (event) {
        event.preventDefault();
        clearErrors('#inventoryForm');

        if (validateInventoryStepTwo()) {
            setInventoryFormStage('step3');
            refreshInventoryPreview();
        }
    });

    $doc.on('click.app', '#inventoryStep3Back', function (event) {
        event.preventDefault();
        setInventoryFormStage('step2');
    });
    ...
    $doc.on('change.app', '#inventoryForm [name="category_select"]', function () {
        syncInventoryCategoryValue();
    });

    $doc.on('input.app', '#inventoryForm [name="custom_category"]', function () {
        syncInventoryCategoryValue();
    });
    ...
    $doc.on('input.app change.app', '#inventoryFilterForm [name="search"], #inventoryFilterForm [name="division"], #inventoryFilterForm [name="stock_status"]', function () {
