...
final class ChartService
{
    ...
    {
        $allowedFields = ['property_type', 'funding_source', 'classification'];
        ...
        $statement = $this->db->prepare(
            sprintf(
                "SELECT COALESCE(NULLIF(TRIM(a.%1\$s), ''), 'Unspecified') AS label, %2\$s AS total
                 FROM assets a
                 INNER JOIN par p ON p.par_id = a.par_id
                 INNER JOIN accountable_officers ao ON ao.officer_id = p.accountable_officer_id%3\$s
                 GROUP BY a.%1\$s
