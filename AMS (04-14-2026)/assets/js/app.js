...
function renderRegistrationTable(rows = appState.registrationOfficers) {
    ...
    if (!officers.length) {
        $('#registrationTableBody').html(`<tr><td colspan="${showUnitColumn ? 6 : 5}" class="px-4 py-10 text-center text-slate-500">No accountable officers found for the current filters.</td></tr>`);
    ...
    $('#assetsDirectoryBody').html(
        paginatedAssets.map((asset, index) => `
            <tr class="${highlightedIds.has(String(asset.property_id || '').trim()) || (highlightedParNumber && String(asset.par_number || '').trim() === highlightedParNumber) ? 'assets-directory-row--highlight' : ''}" data-id="${escapeHtml(asset.id || '')}" data-property-id="${escapeHtml(asset.property_id || '')}" data-par-number="${escapeHtml(asset.par_number || '')}">
                <td class="px-4 py-4 text-center text-slate-600 text-sm" style="width: 4.5rem;">${startIndex + index}</td>
                ...
                <td class="px-4 py-4 text-right">${renderRowViewAction('details-asset', asset.id)}</td>
            </tr>
        `).join('')
    );
...
function renderInventoryTable(rows = appState.inventoryItems) {
    ...
    if (!items.length) {
        $('#inventoryTableBody').html('<tr><td colspan="6" class="px-4 py-10 text-center text-slate-500">No inventory records found for the selected filters.</td></tr>');
    ...
    $('#inventoryTableBody').html(
        paginatedItems.map((item, index) => `
            <tr class="${[
                ...
                <td class="px-4 py-4 text-slate-700">
                    <div class="manage-par-number">${escapeHtml(item.ris_number || 'Pending')}</div>
                </td>
                <td class="px-4 py-4 text-slate-700">
                    <div class="manage-asset-name">${escapeHtml(item.item_name)}</div>
                </td>
                <td class="px-4 py-4 text-slate-700">
                    <div class="manage-par-number">${escapeHtml(item.stock_number || 'Pending')}</div>
                </td>
                <td class="px-4 py-4 text-slate-700">${escapeHtml(String(item.current_stock || 0))}</td>
                <td class="px-4 py-4 text-slate-700">
                    <span class="inventory-status-chip inventory-status-chip--${escapeHtml(String(item.stock_status_code || '').toLowerCase())}">${escapeHtml(item.stock_status_label || 'In Stock')}</span>
                </td>
            </tr>
...
function changePage(section, newPage) {
    ...
    if (section === 'registration') {
        renderRegistrationTable(appState.officerDirectory);
...
$(function () {
    ...
    $doc.on('click.app', '#stockButton', function (event) {
        event.preventDefault();
        event.stopPropagation();
        setInventoryStockMenuVisible($('#stockSubmenu').hasClass('hidden'));
    });

    $doc.on('click.app', '#stockSubmenu [data-action="stock-in"]', function () {
        setInventoryStockMenuVisible(false);
        openInventoryModal();
    });

    $doc.on('click.app', '#stockSubmenu [data-action="stock-out"]', function () {
        const fallbackId = Number((appState.highlightedInventoryIds || [])[0] || 0);
        const item = currentInventoryItem(appState.selectedInventoryItemId) || currentInventoryItem(fallbackId);

        setInventoryStockMenuVisible(false);

        if (!item) {
            showNotice('Select an inventory row first before using Stock Out.', 'error');
            return;
        }
