...
function officerNotificationPayload(action, officer = {}, previousOfficer = null) {
    ...
    const changedFields = previous
        ? fieldKeys.filter((key) => String(previous[key] || '').trim() !== String(current[key] || '').trim())
        : [];

    const fields = fieldKeys.map((key) => ({
        key,
        label: labels[key],
        value: String(current[key] || '').trim() || 'Not provided',
        previousValue: previous ? (String(previous[key] || '').trim() || 'Not provided') : '',
...
function setReportType(reportType, silent = true) {
    ...
    if (normalized === 'SPI' || normalized === 'RPCPPE' || normalized === 'REGSPI') {
        setReportWorkspaceVisible(true);
        $('#spiReportPanel').removeClass('hidden');
        $('#reportSelectionHint').text('SPI report workflow. Set filters and generate the report.');
        setReportPlaceholder('Set filters and generate the SPI report.');

        if (!silent) {
            showNotice('SPI report workflow ready.');
            window.requestAnimationFrame(() => {
                $('#reportWorkflowArea')[0]?.scrollIntoView({ block: 'start', behavior: 'smooth' });
            });
        }
    ...
    if (normalized === 'ICS') {
        setReportWorkspaceVisible(true);
        $('#reportSelectionHint').text(`${normalized} was selected. Its template will follow after SPI.`);
        setReportPlaceholder(`${normalized} report generation is still being configured. Choose PAR or SPI to continue right now.`);

        if (!silent) {
            showNotice(`${normalized} reporting is not ready yet. PAR and SPI are available first.`, 'error');
            window.requestAnimationFrame(() => {
                $('#reportWorkflowArea')[0]?.scrollIntoView({ block: 'start', behavior: 'smooth' });
            });
        }
    ...
    $('#spiReportPanel').addClass('hidden');
...
function openEditModal(asset) {
    ...
    $('#editAssetForm [name="unit_cost"]').val(asset.unit_cost || '');
    ...
    $('#editAssetForm [name="officer_id"]').val(asset.officer_id || '');
