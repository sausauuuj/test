...
final class OfficerService
{
    ...
    public function listFiltered(array $filters = []): array
    {
        ...
        $where = [];
        $params = [];
        $divisionOrder = implode(', ', array_map(
        ...
        $statement = $this->db->prepare(
            'SELECT officer_id, name, position, unit, division, created_at, updated_at
             FROM accountable_officers' .
             ($where === [] ? '' : ' WHERE ' . implode(' AND ', $where)) .
             ' ORDER BY FIELD(division, ' . $divisionOrder . '), name ASC'
        );
    ...
    public function create(array $payload, bool $requireProfile = true): array
    {
        ...
        $statement = $this->db->prepare(
            'INSERT INTO accountable_officers (name, position, unit, division)
             VALUES (:name, :position, :unit, :division)'
        );
        $statement->execute([
            'name' => $data['name'],
    ...
    {
        if (!$this->isValidDivision($division)) {
    ...
    private function isValidDivision(string $division): bool
    {
    ...
    private function validateOfficerPayload(array $payload, bool $requireProfile): array
    {
        ...
        } elseif (!$this->isValidDivision($division)) {
