...
final class ReportService
{
    ...
    private const PAR_ISSUED_BY_ROLE = 'ADAS III - FAD/PRMS';
    ...
    private function renderRpcppeHtml(array $assets, array $filters): string
    {
        ...
        $asOfDate = trim((string) ($filters['par_date_to'] ?? $filters['date_to'] ?? ''));
        $asOfLabel = $asOfDate !== '' ? $this->formatLongDate($asOfDate) : $this->formatLongDate(date('Y-m-d'));
        $entityName = str_contains(strtoupper($fundingSource), 'RDC')
            ? 'REGIONAL DEVELOPMENT COUNCIL IX'
            : 'Department of Economy, Planning, and Development IX';
        $propertyOfficer = $this->resolveIssuedByProfile();

        $html = '<article class="rpcppe-report-page">';
        $html .= '<div class="rpcppe-report-head">';
        $html .= '<h2>REPORT ON THE PHYSICAL COUNT OF PROPERTY/PLANT AND EQUIPMENT</h2>';
        $html .= '<div class="rpcppe-report-rule">--------------------------------------------------</div>';
        $html .= '<p>' . escape_html($entityName) . '</p>';
        $html .= '<h3>' . escape_html($propertyType !== '' ? $propertyType : 'PROPERTY, PLANT AND EQUIPMENT') . '</h3>';
        $html .= '<p>Made as of ' . escape_html($asOfLabel) . '</p>';
        $html .= '<div class="rpcppe-report-accountable">';
        $html .= '<span>For which</span>';
        $html .= '<strong>' . escape_html($propertyOfficer['name']) . ',</strong>';
        $html .= '<strong>' . escape_html($propertyOfficer['role']) . ',</strong>';
        $html .= '<span>NEDA, Regional Office IX, is accountable, having assumed such accountability on November 10, 2023.</span>';
        $html .= '</div>';
        $html .= '<div class="rpcppe-report-sign-line"><span>(Name of accountable officer)</span><span>(Official Designation)</span><span>(Bureau or Office)</span></div>';
        $html .= '</div>';
        $html .= '<table class="rpcppe-report-table">';
        $html .= '<colgroup>';
        foreach ([7, 6, 12, 7, 7, 24, 7, 7, 7, 4, 4, 5, 5, 8] as $width) {
        ...
        $html .= '<th>FUND</th><th>QUANTITY</th><th>QTY PER COUNT</th><th>SHORTAGE/OVERAGE</th><th>REMARKS</th>';
        $html .= '</tr></thead><tbody>';
        ...
        $html .= '<tr class="rpcppe-signature-row"><td colspan="3">PREPARED BY:</td><td colspan="3">CERTIFIED CORRECT BY:</td><td colspan="4">APPROVED BY:</td><td colspan="4">VERIFIED BY:</td></tr>';
        $html .= '<tr class="rpcppe-signature-name"><td colspan="3">KRISTY R. MANTE</td><td colspan="3">GREEGIE M. ALAYON</td><td colspan="4">MARIA FELICIDAD R. GUERRERO</td><td colspan="4">EDITHO P. BUCOL</td></tr>';
        $html .= '</tbody></table>';
        $html .= '</article>';
    ...
    private function groupAssetsByPar(array $assets): array
    {
        ...
        foreach ($assets as $asset) {
            ...
            $groupKey = implode('|', [
                $documentType,
                trim((string) ($asset['par_id'] ?? '')),
                trim((string) ($asset['par_number'] ?? '')),
    ...
    private function renderParHtml(array $sheets, array $filters): string
    {
        ...
        $pendingHalfPageSheets = [];

        foreach ($sheets as $sheet) {
            if ($this->isFullPageSheet($sheet)) {
                if ($pendingHalfPageSheets !== []) {
                    $html .= '<div class="par-report-multi-copy-page">';

                    foreach ($pendingHalfPageSheets as $pendingSheet) {
                        $html .= $this->renderParSheet($pendingSheet);
                    }
                ...
                $html .= '<div class="par-report-multi-copy-page par-report-multi-copy-page--single">';
                $html .= $this->renderParSheet($sheet);
                $html .= '</div>';
                continue;
            }

            $pendingHalfPageSheets[] = $sheet;

            if (count($pendingHalfPageSheets) === 2) {
                $html .= '<div class="par-report-multi-copy-page">';
        ...
        $html .= $this->renderAssetStickerPages($sheets);

        $html .= '</div>';
        return $html;
    }

    private function renderAssetStickerPages(array $sheets): string
    {
        $assets = [];

        foreach ($sheets as $sheet) {
            if ($this->documentType($sheet) === 'ICS') {
                continue;
            }
        ...
        $html = '<div class="asset-sticker-stack">';

        foreach (array_chunk($assets, 8) as $assetPage) {
            $html .= '<section class="asset-sticker-page">';

            foreach ($assetPage as [$sheet, $asset]) {
                $html .= $this->renderAssetSticker($sheet, $asset);
            }
    ...
    private function renderAssetSticker(array $sheet, array $asset): string
    {
        $officerOffice = $this->buildSignatureOfficeLabel($sheet);
        $descriptionLines = array_values(array_filter([
            ...
            trim((string) ($asset['property_id'] ?? '')) !== ''
                ? 'SN: ' . trim((string) ($asset['property_id'] ?? ''))
                : '',
        ], static fn (string $value): bool => $value !== ''));

        $html = '<article class="asset-sticker">';
        $html .= '<div class="asset-sticker__head">';
        $html .= '<div class="asset-sticker__head-text">';
        $html .= '<div class="asset-sticker__agency">DEPARTMENT OF ECONOMY, PLANNING, AND DEVELOPMENT</div>';
        $html .= '<div class="asset-sticker__office">REGIONAL OFFICE IX, PAGADIAN CITY</div>';
        $html .= '</div>';
        $html .= '<img class="asset-sticker__logo" src="assets/images/DEPDev_Logo_High-res.svg.png" alt="DEPDev logo">';
        $html .= '</div>';
        $html .= '<div class="asset-sticker__category">' . escape_html(strtoupper((string) ($asset['property_type'] ?? ''))) . '</div>';
        $html .= '<div class="asset-sticker__cell-group">';
        $html .= '<div class="asset-sticker__row"><span>Division:</span><strong>' . escape_html((string) ($sheet['division'] ?? '')) . '</strong></div>';
        $html .= '<div class="asset-sticker__row"><span>Accountable Officer:</span><strong>' . escape_html($this->formatSignatureName((string) ($sheet['officer_name'] ?? ''))) . '</strong></div>';
        $html .= '<div class="asset-sticker__row"><span>Position/Office:</span><strong>' . escape_html($officerOffice) . '</strong></div>';
        $html .= '<div class="asset-sticker__row asset-sticker__row--description"><span>Description:</span><strong>' . escape_html(implode("\n", $descriptionLines)) . '</strong></div>';
        $html .= '</div>';
        $html .= '<div class="asset-sticker__details">';
        $html .= '<div class="asset-sticker__row"><span>Property No.:</span><strong>' . escape_html((string) ($asset['property_number'] ?? '')) . '</strong></div>';
        $html .= '<div class="asset-sticker__row"><span>Date Acquired:</span><strong>' . escape_html($this->formatDate((string) ($asset['date_acquired'] ?? ''))) . '</strong></div>';
        $html .= '<div class="asset-sticker__row"><span>Unit:</span><strong>unit</strong></div>';
        $html .= '<div class="asset-sticker__row"><span>Source:</span><strong>' . escape_html($this->normalizeFundingSourceLabel((string) ($asset['funding_source'] ?? ''))) . '</strong></div>';
        $html .= '<div class="asset-sticker__row"><span>Acquisition Cost:</span><strong>P ' . escape_html(number_format((float) ($asset['unit_cost'] ?? 0), 2)) . '</strong></div>';
        $html .= '</div>';
        $html .= '<div class="asset-sticker__signatures">';
        $html .= '<div><span></span><strong>(COA Representative)</strong></div>';
        $html .= '<div><span>' . escape_html($this->formatSignatureName((string) ($sheet['officer_name'] ?? ''))) . '</span><strong>(Property Custodian)</strong></div>';
        $html .= '</div>';
        $html .= '<div class="asset-sticker__warning">PLEASE DON\'T TAKE THIS OFF!</div>';
        $html .= '</article>';
    ...
    private function renderParSheet(array $sheet): string
    {
        ...
        if ($this->isFullPageSheet($sheet)) {
            $classes[] = 'par-report-page--full';
        }

        if ($this->isDenseSheet($sheet)) {
            $classes[] = 'par-report-page--dense';
        }

        if ($this->isUltraDenseSheet($sheet)) {
            $classes[] = 'par-report-page--ultra-dense';
        }

        $html = '<article class="' . escape_html(implode(' ', $classes)) . '">';
        $html .= '<table class="par-sheet-table">';
        $html .= '<colgroup>';
        $html .= '<col style="width:6.69%">';
        $html .= '<col style="width:4.71%">';
        $html .= '<col style="width:50.99%">';
        $html .= '<col style="width:12.06%">';
        $html .= '<col style="width:13.38%">';
        $html .= '<col style="width:12.17%">';
        $html .= '</colgroup>';
        $html .= '<tbody>';
        ...
        $html .= '<td class="par-sheet-table__meta-label">Date :</td>';
        ...
        foreach ($items as $itemIndex => $asset) {
            ...
            $html .= '<td class="par-sheet-table__cell"><div class="par-sheet-table__description">' . escape_html($this->buildAssetDescription($asset)) . '</div></td>';
        ...
        $html .= '<tr class="par-sheet-table__blank-row par-sheet-table__signature-gap"><td colspan="3"></td><td colspan="3"></td></tr>';
        ...
        $html .= '</tr>';
        $html .= '<tr class="par-sheet-table__signature-row par-sheet-table__signature-row--name">';
        $html .= '<td colspan="3" class="par-sheet-table__signature-cell par-sheet-table__signature-text par-sheet-table__signature-text--line par-sheet-table__signature-text--strong">' . escape_html($receivedProfile['name']) . '</td>';
        $html .= '<td colspan="3" class="par-sheet-table__signature-cell par-sheet-table__signature-text par-sheet-table__signature-text--line par-sheet-table__signature-text--strong">' . escape_html($issuedProfile['name']) . '</td>';
        $html .= '</tr>';
        $html .= '<tr class="par-sheet-table__signature-row par-sheet-table__signature-row--caption">';
        $html .= '<td colspan="3" class="par-sheet-table__signature-cell par-sheet-table__signature-text">Signature over Printed Name of Accountable Officer</td>';
        $html .= '<td colspan="3" class="par-sheet-table__signature-cell par-sheet-table__signature-text">Signature over Printed Name of Issuing Officer</td>';
        $html .= '</tr>';
        $html .= '<tr class="par-sheet-table__signature-row par-sheet-table__signature-row--role">';
        $html .= '<td colspan="3" class="par-sheet-table__signature-cell par-sheet-table__signature-text par-sheet-table__signature-text--line">' . escape_html($receivedProfile['role']) . '</td>';
        $html .= '<td colspan="3" class="par-sheet-table__signature-cell par-sheet-table__signature-text par-sheet-table__signature-text--line">' . escape_html($issuedProfile['role']) . '</td>';
        $html .= '</tr>';
        $html .= '<tr class="par-sheet-table__signature-row par-sheet-table__signature-row--caption">';
        $html .= '<td colspan="3" class="par-sheet-table__signature-cell par-sheet-table__signature-text">Position/Office</td>';
        $html .= '<td colspan="3" class="par-sheet-table__signature-cell par-sheet-table__signature-text">Position/Office</td>';
        $html .= '</tr>';
        $html .= '<tr class="par-sheet-table__signature-row par-sheet-table__signature-row--date">';
        $html .= '<td colspan="3" class="par-sheet-table__signature-cell par-sheet-table__signature-text par-sheet-table__signature-text--line">' . escape_html($date) . '</td>';
        $html .= '<td colspan="3" class="par-sheet-table__signature-cell par-sheet-table__signature-text par-sheet-table__signature-text--line">' . escape_html($date) . '</td>';
        $html .= '</tr>';
        $html .= '<tr class="par-sheet-table__signature-row par-sheet-table__signature-row--caption par-sheet-table__signature-row--last">';
        $html .= '<td colspan="3" class="par-sheet-table__signature-cell par-sheet-table__signature-text">Date</td>';
        $html .= '<td colspan="3" class="par-sheet-table__signature-cell par-sheet-table__signature-text">Date</td>';
        $html .= '</tr>';
        $html .= '</tbody></table>';
        $html .= '</article>';
    ...
    private function renderParCsv(array $sheets): string
    {
        ...
        foreach ($sheets as $index => $sheet) {
            ...
            fputcsv($stream, ['Fund Cluster:', $this->normalizeFundingSourceLabel((string) ($sheet['funding_source'] ?? '')), '', '', 'Date :', $this->formatLongDate((string) ($sheet['par_date'] ?? ''))], ',', '"', '\\');
    ...
    private function prepareDocumentFilters(array $filters, string $documentType): array
    {
        ...
        $requestedClassification = strtoupper(trim((string) ($filters['classification'] ?? '')));
        $expectedClassification = $documentType === 'ICS' ? 'SEMI' : 'PPE';
        $filters['classification'] = $requestedClassification !== '' ? $requestedClassification : $expectedClassification;

        if ($filters['classification'] !== $expectedClassification) {
            throw new ValidationException('Select a matching classification for the report type.', [
                'classification' => 'PPE generates PAR and SEMI generates ICS.',
            ]);
        }

        if (trim((string) ($filters['date_from'] ?? '')) !== '') {
            $filters['par_date_from'] = trim((string) $filters['date_from']);
            unset($filters['date_from']);
        }

        if (trim((string) ($filters['date_to'] ?? '')) !== '') {
            $filters['par_date_to'] = trim((string) $filters['date_to']);
    ...
    private function renderSpreadsheetWorksheetXml(array $sheet, int $sheetIndex): string
    {
        ...
        $rows[] = $this->spreadsheetRowXml([
            ...
            $this->spreadsheetCellXml('Date :', 'MetaLabelCenter'),
    ...
    private function isFullPageSheet(array $sheet): bool
    {
        return count($sheet['items'] ?? []) > 3 || $this->estimateSheetVisualLines($sheet) > 14;
    }

    private function isDenseSheet(array $sheet): bool
    {
        return count($sheet['items'] ?? []) > 2 || $this->estimateSheetVisualLines($sheet) > 10;
    }

    private function isUltraDenseSheet(array $sheet): bool
    {
        return count($sheet['items'] ?? []) > 5 || $this->estimateSheetVisualLines($sheet) > 24;
    }

    private function estimateSheetVisualLines(array $sheet): int
    {
        $visualLineCount = 0;

        foreach ($sheet['items'] ?? [] as $asset) {
            $description = $this->buildAssetDescription($asset);
            $lines = preg_split('/\n/', str_replace(["\r\n", "\r"], "\n", $description)) ?: [''];

            foreach ($lines as $line) {
                $length = function_exists('mb_strlen') ? mb_strlen($line) : strlen($line);
                $visualLineCount += max(1, (int) ceil($length / 54));
            }
        }
    ...
    private function buildSignatureOfficeLabel(array $sheet): string
    {
        ...
        $unit = $this->normalizeOfficerProfilePart($sheet['officer_unit'] ?? '');
        $division = $this->normalizeOfficerProfilePart($sheet['division'] ?? '');
        $office = $division;

        if (strtoupper($division) === 'FAD' && $unit !== '' && strtoupper($unit) !== 'FAD') {
            $office = 'FAD/' . $unit;
        }
        ...
        return $position !== '' ? $position : $office;
    }
    ...
    private function resolveSignatureProfile(array $sheet, string $mode): array
    {
        ...
        return [
            'name' => $this->formatSignatureName((string) ($sheet['officer_name'] ?? '')),
    ...
    private function resolveIssuedByProfile(): array
    {
        ...
        $this->issuedByProfile = [
            'name' => $this->formatSignatureName((string) ($officer['name'] ?? self::PAR_ISSUED_BY_NAME)),
            'role' => self::PAR_ISSUED_BY_ROLE,
        ];
    ...
    private function formatSignatureName(string $name): string
    {
        ...
        if (str_contains($normalized, ',')) {
            [$lastName, $givenNames] = array_map('trim', explode(',', $normalized, 2));
            $normalized = trim($givenNames . ' ' . $lastName);
        }

        $parts = preg_split('/\s+/', $normalized) ?: [];

        return trim(implode(' ', array_map(static function (string $part): string {
