...
final class AssetService
{
    ...
    public function normalizeFilters(array $filters): array
    {
        return [
            ...
            'par_date_from' => trim((string) ($filters['par_date_from'] ?? '')),
            'par_date_to' => trim((string) ($filters['par_date_to'] ?? '')),
    ...
    public function buildWhereClause(array $filters, string $assetAlias = 'a', string $parAlias = 'p'): array
    {
        ...
        if ($filters['par_date_from'] !== '') {
            $where[] = sprintf('%s.par_date >= :par_date_from', $parAlias);
            $params['par_date_from'] = $filters['par_date_from'];
        }

        if ($filters['par_date_to'] !== '') {
            $where[] = sprintf('%s.par_date <= :par_date_to', $parAlias);
            $params['par_date_to'] = $filters['par_date_to'];
        }
    ...
    {
        [$whereClause, $params] = $this->buildWhereClause($filters);
        $totalOfficers = (int) $this->db->query('SELECT COUNT(*) FROM accountable_officers')->fetchColumn();
        ...
        $metrics['total_officers'] = $totalOfficers;
    ...
    public function updateSerialNumber(int $assetId, array $payload): array
    {
        ...
        if ($propertyId === '') {
            throw new ValidationException('Please review the serial number.', [
                'property_id' => 'Serial number is required.',
            ]);
        }

        if ($this->propertyIdExistsForOtherAsset($propertyId, $assetId)) {
            throw new ValidationException('Please review the serial number.', [
                'property_id' => 'Serial number already exists in the inventory.',
            ]);
        }
