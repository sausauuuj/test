...
{
    public const REQUEST_TYPES = ['ISM', 'OSMI'];
    ...
    public function listItems(array $filters = [], int $limit = 250): array
    {
        ...
        $requestType = $this->normalizeRequestType((string) ($filters['request_type'] ?? ($filters['item_type'] ?? '')));
        ...
        if (in_array($requestType, self::REQUEST_TYPES, true)) {
            $where[] = $requestType === 'ISM'
                ? 'ii.request_type IN (:request_type, :legacy_request_type)'
                : 'ii.request_type = :request_type';
            $params['request_type'] = $requestType;
            if ($requestType === 'ISM') {
                $params['legacy_request_type'] = 'RSMI';
            }
        }
    ...
    public function preview(array $payload): array
    {
        ...
        $requestType = $this->normalizeRequestType((string) ($payload['request_type'] ?? ($existing['request_type'] ?? '')));
    ...
    public function deleteStockOutMovement(int $movementId): array
    {
        if ($movementId <= 0) {
            throw new ValidationException('The selected stock out record could not be found.', [
                'movement_id' => 'Choose a valid stock out record.',
            ]);
        }
        ...
        if (!$movement || strtoupper((string) ($movement['movement_type'] ?? '')) !== 'DEDUCT') {
            throw new ValidationException('The selected stock out record could not be found.', [
                'movement_id' => 'Choose a valid stock out record.',
            ]);
        }

        $itemId = (int) ($movement['inventory_item_id'] ?? 0);
        $quantity = max(0, (int) ($movement['quantity'] ?? 0));
        $currentStock = max(0, (int) ($movement['current_stock'] ?? 0));
        $stockLimit = max(0, (int) ($movement['stock_limit'] ?? 0));
        $quantityIssued = max(0, (int) ($movement['quantity_issued'] ?? 0));
        $restoredStock = $currentStock + $quantity;
        $restoredLimit = max($stockLimit, $quantityIssued, $restoredStock);
        ...
        try {
            ...
            $update->execute([
                ...
                'stock_limit' => $restoredLimit,
                'quantity_issued' => max($quantityIssued, $restoredLimit),
                'low_stock_threshold' => max(1, (int) ceil($restoredLimit * 0.2)),
                'inventory_item_id' => $itemId,
            ]);
    ...
    {
        $requestType = $this->normalizeRequestType((string) ($payload['request_type'] ?? ''));
        $division = strtoupper(trim((string) ($payload['division'] ?? '')));
        $officerId = (int) ($payload['officer_id'] ?? 0);
        $category = trim((string) ($payload['category'] ?? ''));
        ...
        $allocationLimit = $existing !== null
            ? max(
                $currentStock,
                (int) ($existing['stock_limit'] ?? 0),
                (int) ($existing['quantity_issued'] ?? 0)
            )
            : $quantityIssued;
        ...
        if (!in_array($requestType, self::REQUEST_TYPES, true)) {
            $errors['request_type'] = 'Choose either ISM or OSMI.';
        }
    ...
    {
        $normalized = strtoupper(trim($requestType));
    ...
    private function hydrateItem(array $item): array
    {
        ...
        $item['request_type'] = $this->normalizeRequestType((string) ($item['request_type'] ?? '')) ?: 'ISM';
