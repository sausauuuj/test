...
function triggerReportPrint() {
    if (!['PAR', 'ICS'].includes(appState.reportType) || !appState.reportReady) {
        return;
    }
...
function renderInventoryAlerts(summary = {}) {
    ...
    $meta.text(`LOW: ${lowCount} | NEAR LOW: ${nearCount}`);
        ...
        watchlist.map((item) => `
            <article class="inventory-alert-item inventory-alert-item--${escapeHtml(String(item.stock_status_code || '').toLowerCase())}">
                <div class="inventory-alert-item__main">
                    ...
                    <p class="inventory-alert-item__meta">${escapeHtml(item.item_code)} | ${escapeHtml(item.item_type)} | ${escapeHtml(item.unit)}</p>
                </div>
                <div class="inventory-alert-item__stats">
                    <strong>${escapeHtml(String(item.current_stock || 0))}</strong>
                    <span>Current / Low ${escapeHtml(String(item.low_stock_threshold || 0))}</span>
                </div>
            </article>
        `).join('')
    );
...
function validateAssetDraft(payload) {
    ...
    if ((payload.estimated_useful_life || '').trim() === '') {
...
function renderRegistrationTable(rows = appState.registrationOfficers) {
    ...
    if (!officers.length) {
        $('#registrationTableBody').html(`<tr><td colspan="${showUnitColumn ? 5 : 4}" class="px-4 py-10 text-center text-slate-500">No accountable officers found for the current filters.</td></tr>`);
        return officers;
    }
    ...
    $('#assetsDirectoryBody').html(
        assets.map((asset) => `
            <tr class="${highlightedIds.has(String(asset.property_id || '').trim()) || (highlightedParNumber && String(asset.par_number || '').trim() === highlightedParNumber) ? 'assets-directory-row--highlight' : ''}" data-id="${escapeHtml(asset.id || '')}" data-property-id="${escapeHtml(asset.property_id || '')}" data-par-number="${escapeHtml(asset.par_number || '')}">
                ...
                <td class="px-4 py-4 text-slate-700">${escapeHtml(asset.funding_source)}</td>
                <td class="px-4 py-4 text-slate-700"><span class="classification-chip classification-chip--${escapeHtml(String(asset.classification || '').trim().toLowerCase())}">${escapeHtml(asset.classification || 'N/A')}</span></td>
                <td class="px-4 py-4 text-slate-700">${escapeHtml(asset.estimated_useful_life || '')}</td>
                <td class="px-4 py-4 text-slate-700">${escapeHtml(asset.current_condition || '')}</td>
            </tr>
        `).join('')
    );
...
function renderInventoryTable(rows = appState.inventoryItems) {
    ...
    const lowCount = items.filter((item) => String(item.stock_status_code || '') === 'LOW').length;
    const nearCount = items.filter((item) => String(item.stock_status_code || '') === 'NEAR').length;

    $('#inventoryTableMeta').text(formatRecordCountLabel(items.length));
    $('#inventorySummaryMeta').text(`LOW STOCK: ${lowCount} | NEAR LOW: ${nearCount}`);

    if (!items.length) {
        $('#inventoryTableBody').html('<tr><td colspan="10" class="px-4 py-10 text-center text-slate-500">No supplies or materials found for the selected filters.</td></tr>');
        return items;
    }

    $('#inventoryTableBody').html(
        items.map((item) => `
            <tr class="${highlightedIds.has(Number(item.inventory_item_id || 0)) ? 'inventory-table__row--highlight' : ''}" data-id="${escapeHtml(item.inventory_item_id || '')}">
                <td class="px-4 py-4 text-slate-700">
                    <div class="manage-par-number">${escapeHtml(item.item_code)}</div>
                    <div class="manage-par-meta">${escapeHtml(item.item_type)}</div>
                </td>
                <td class="px-4 py-4 text-slate-700">
                    <div class="manage-asset-name">${escapeHtml(item.item_name)}</div>
                    <div class="manage-asset-meta">${escapeHtml(item.description || 'No notes provided')}</div>
                </td>
                <td class="px-4 py-4 text-slate-700">${escapeHtml(item.item_type)}</td>
                <td class="px-4 py-4 text-slate-700">${escapeHtml(item.unit)}</td>
                <td class="px-4 py-4 text-slate-700"><strong>${escapeHtml(String(item.current_stock || 0))}</strong></td>
                <td class="px-4 py-4 text-slate-700">${escapeHtml(String(item.stock_limit || 0))}</td>
                <td class="px-4 py-4 text-slate-700">${escapeHtml(String(item.low_stock_threshold || 0))}</td>
                <td class="px-4 py-4 text-slate-700">
                    <span class="inventory-status-chip inventory-status-chip--${escapeHtml(String(item.stock_status_code || '').toLowerCase())}">${escapeHtml(item.stock_status_label || 'In Stock')}</span>
                    <div class="manage-par-meta mt-2">${escapeHtml(item.stock_remark || 'Stock level is healthy.')}</div>
                </td>
                <td class="px-4 py-4 text-slate-700">${escapeHtml(formatInventoryTimestamp(item.updated_at))}</td>
                <td class="px-4 py-4 text-right">
                    <div class="manage-actions">
                        <button type="button" class="manage-action-icon manage-action-icon--info details-inventory" data-id="${item.inventory_item_id}" title="View details" aria-label="View details">
                            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M2.5 12s3.6-6 9.5-6 9.5 6 9.5 6-3.6 6-9.5 6-9.5-6-9.5-6Z"></path><circle cx="12" cy="12" r="2.6"></circle></svg>
                        </button>
                        <button type="button" class="manage-action-icon manage-action-icon--info edit-inventory" data-id="${item.inventory_item_id}" title="Edit item" aria-label="Edit item">
                        ...
                        <button type="button" class="manage-action-icon manage-action-icon--info add-inventory-stock" data-id="${item.inventory_item_id}" title="Add stock" aria-label="Add stock">
                        ...
                        <button type="button" class="manage-action-icon manage-action-icon--danger deduct-inventory-stock" data-id="${item.inventory_item_id}" title="Deduct stock" aria-label="Deduct stock">
                            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M5 12h14"></path></svg>
                        </button>
                    </div>
                </td>
            </tr>
...
function resetInventoryForm() {
    ...
    $form.find('[name="current_stock"]').prop('disabled', false).closest('.form-group').removeClass('opacity-70');
    clearErrors('#inventoryForm');
    $('#inventoryModalTitle').text('Add Supply or Material');
...
function openInventoryModal(item = null) {
    ...
    if (item) {
        $('#inventoryModalTitle').text('Update Supply or Material');
        $('#inventoryForm [name="inventory_item_id"]').val(item.inventory_item_id || '');
        $('#inventoryForm [name="item_name"]').val(item.item_name || '');
        $('#inventoryForm [name="item_type"]').val(item.item_type || '');
        $('#inventoryForm [name="unit"]').val(item.unit || '');
        $('#inventoryForm [name="current_stock"]').val(item.current_stock || 0).prop('disabled', true).closest('.form-group').addClass('opacity-70');
        $('#inventoryForm [name="stock_limit"]').val(item.stock_limit || 0);
        $('#inventoryForm [name="low_stock_threshold"]').val(item.low_stock_threshold || 0);
...
function openInventoryMovementModal(item, movementType) {
    ...
    $('#inventoryMovementSummary').html(`
        ...
        <div class="inventory-summary-card__item">
            <span class="inventory-summary-card__label">Low Stock No.</span>
            <strong class="inventory-summary-card__value">${escapeHtml(String(item.low_stock_threshold || 0))}</strong>
        </div>
        <div class="inventory-summary-card__item">
            <span class="inventory-summary-card__label">Limit</span>
...
function renderInventoryDetails(details = {}) {
    ...
    $('#inventoryDetailsMeta').text(`${item.item_code || ''} | ${item.item_type || ''} | ${item.unit || ''}`.replace(/^\s+\|\s+|\s+\|\s+$/g, '').trim());
    $('#inventoryDetailsContent').html(`
        <section class="detail-section">
            <div class="detail-section__head">
                <p class="panel-eyebrow">Stock Summary</p>
                <h4 class="panel-title">Current inventory level</h4>
            </div>
            <div class="detail-grid mt-4">
                ...
                <div class="detail-item"><div class="detail-label">Low Stock No.</div><div class="detail-value">${escapeHtml(String(item.low_stock_threshold || 0))}</div></div>
...
function refreshInventoryView(silent = true) {
    ...
    return apiRequest('api/inventory/list.php', 'GET', filters)
        .done((response) => {
            ...
            if (!silent) {
                showNotice(hasFilters ? 'Inventory filters applied.' : 'Showing all supplies and materials.');
            }
        })
        .fail((xhr) => {
...
function refreshManagementView(silent = true) {
    return refreshAssetsDirectory(silent);
}
...
$(function () {
                ...
                apiRequest('api/assets/update.php', 'POST', requestPayload)
                    .done((response) => {
                        ...
                        notifyTransaction(response.message || 'PAR assets updated successfully.', 'success', {
                            category: 'Assets',
                            details: `Updated ${response.data?.updated_count || updatedAssets.length || 0} asset record${(response.data?.updated_count || updatedAssets.length || 0) === 1 ? '' : 's'} under the selected PAR. Serial numbers were preserved.`,
                        });
                        resetAssetWorkflow(true);
                        setAssetEntryVisible(false);
    ...
    $doc.on('submit.app', '#inventoryForm', function (event) {
        ...
        const successMessage = hasId ? 'Inventory item updated successfully.' : 'Inventory item saved successfully.';
        ...
        apiRequest(endpoint, 'POST', payload)
            .done((response) => {
                ...
                notifyTransaction(response.message || successMessage, 'success', {
                    category: 'Inventory',
                    details: hasId
                        ? 'The selected supply or material was updated and the stock controls were refreshed.'
                        : 'A new supply or material was added and placed at the top of the inventory table.',
                });
                closeInventoryModal();
            ...
            .fail((xhr) => {
                handleRequestError(xhr, hasId ? 'Unable to update the inventory item.' : 'Unable to save the inventory item.', '#inventoryForm');
            });
    });

    $doc.on('input.app change.app', '#inventoryFilterForm [name="search"], #inventoryFilterForm [name="item_type"], #inventoryFilterForm [name="stock_status"]', function () {
    ...
    $doc.on('click.app', '#inventoryTableBody .details-inventory', function () {
    ...
    $doc.on('keydown.app', function (event) {
        if ((event.ctrlKey || event.metaKey) && String(event.key || '').toLowerCase() === 'p') {
            if (appState.activeView === 'reports' && ['PAR', 'ICS'].includes(appState.reportType) && appState.reportReady) {
                event.preventDefault();
                triggerReportPrint();
                return;
            }
